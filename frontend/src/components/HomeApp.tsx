  ...
  return (
    <main className="sky-wash min-h-screen">
      <header className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-6 py-6">
        ...
        <nav className="flex flex-wrap items-center gap-3 text-sm font-medium text-ink/75 md:gap-4">
          <Link href="/about" className="hover:text-ink">
            About
          </Link>
          <Link href="/gun-milan" className="hover:text-ink">
            Gun Milan
          </Link>
      ...
      {/* Crawlable English summary for search engines (also in SSR page shell) */}
      <section className="border-t border-ink/10 bg-paper/80 px-6 py-12">
        <div className="mx-auto max-w-3xl space-y-4 text-ink/75">
          <h2 className="font-display text-3xl font-semibold text-ink">
            Auditable Vedic astrology software
          </h2>
          <p>
            AstroOS is a Vedic Astrology Operating System for birth charts, Gun Milan
            (Ashtakoot), marriage timing themes, and evidence-backed explanations. Every
            conclusion follows Evidence → Rule → Classical Source → AI Explanation →
            Confidence. AI never invents planetary facts or definite predictions.
          </p>
          <p>
            Explore{" "}
            <Link href="/gun-milan" className="text-copper underline">
              Gun Milan
            </Link>{" "}
            and{" "}
            <Link href="/about" className="text-copper underline">
              About AstroOS
            </Link>
            .
          </p>
